...
### 틀렸던 이유
1번째 제출 실패 : wine,dp 인덱스가 1부터 시작해야 3번째 전까지의 max 값을 더해줄 수 있음
dp의 크기는 N+1이고 N번째 인덱스까지 for 문이 돌아야 마지막 잔까지 고려할 수 있음
2번째 제출 실패 : N는 1이상임. 1일 때 고려안해서 런타임에러(indexError)남
dp[2] = wine[1] + wine[2]는 N이 2이상일 때만 가능
